  ...
  return (
    <div className="space-y-5">
      <InfoBanner storageKey="bot-knowledge">
        ...
        <p className="font-semibold mt-3 mb-1">AIの検索のしくみ（セマンティック検索）</p>
        <p>このシステムは<span className="font-medium">意味ベースの検索（セマンティック検索）</span>を採用しています。キーワードが完全一致しなくても、意味が近い質問に自動でマッチします。</p>
        <ul className="space-y-0.5 list-none mt-1">
          <li>例：「分割払いできますか？」→「医療ローン」の知識がヒット</li>
          <li>例：「お金が心配で…」→「費用・支払い方法」の知識がヒット</li>
          <li>例：「痛い？」→「ダウンタイム・副作用」の知識がヒット</li>
        </ul>

        <p className="font-semibold mt-3 mb-1">カテゴリの活用</p>
        <p>「施術説明」「よくある質問」「アクセス」「料金・費用」などカテゴリを分けて登録すると管理しやすくなります。カテゴリ名は自由に設定できます。</p>

        <p className="font-semibold mt-3 mb-1">登録のコツ</p>
        <ul className="space-y-0.5 list-none">
          <li>・タイトルは短く、内容を一言で表す（例：「ダーマペンの効果と向いている人」）</li>
          <li>・本文はお客様に話しかけるような自然な文体で書く</li>
          <li>・キーワードには類義語・よくある誤字も入れておくと検索精度がさらに上がる</li>
          <li>・1件に詰め込みすぎず、テーマごとに分けて登録するのがおすすめ</li>
          <li>・登録した直後から自動でAIが学習します（手動の再起動不要）</li>
        </ul>
      </InfoBanner>
