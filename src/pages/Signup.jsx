...
export default function Signup() {
  ...
  useEffect(() => {
    axios.get(`${BASE}/api/clinics/signup/${token}`)
  ...
  const submit = async (e) => {
    ...
    try {
      await axios.post(`${BASE}/api/clinics/signup/${token}`, { password })
      setDone(true)
    } catch (e) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-600 via-primary-700 to-accent-600 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-primary-200 text-sm mt-1">クリニック登録</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          {done ? (
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-gray-600">登録が完了しました。ログイン画面からサインインしてください。</p>
              <button onClick={() => navigate('/login')} className="w-full py-2.5 bg-primary-600 text-white rounded-lg font-medium text-sm hover:bg-primary-700 transition-colors">
                ログインする
              </button>
            </div>
          ) : error && !invitation ? (
            ...
            <>
              <h2 className="text-xl font-bold text-gray-900 mb-1">パスワード設定</h2>
              {invitation && (
                <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                  <p className="text-xs text-gray-500">クリニック名</p>
                  <p className="text-sm font-medium text-gray-900">{invitation.clinic_name}</p>
